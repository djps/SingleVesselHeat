...
// This file contains the implementation of the "Multiply by 1 + t/2 L_2" function. 
// It's a long and complicated function, so I've stuck it in a separate header. 
// All these headers will be included into run_heat_solver.cpp and that can be 
// compiled into a single object file. No need to have too many object files running around. 
...
{
	// This routine steps through y and z, calculating the matrix (1+0.5*dt*L_2), where L_2 is the 
	// matrix of the operator -vd/dy + kappa d^2/dy^2. 
    // To ensure that boundaries are correctly accounted for, we do separate sums to the different 
	// indices in the conductivity kappa.    
	...
	*/
    ...
    for (int x = 0; x<nx; ++x)
    {
        ...
        { 
			...
			// resolution of spatial derivatives at these points. So instead of simply looping over x, we 
			// loop first over the indices between boundaries and then deal with the boundaries. Note that 
			// each internal boundary forces 2 points to be dealt with specially. 
			...
			for(int counter = 0; counter < M-1;++counter) // M is defined in conductivity.h Basically, this loop loops over all discontinuities, so within each loop we need only worry about continuous bits plus the two boundary terms.    
            {
				...
				// boundary. So actually, I'm now running over all the elements that will be done without 
...
/*             if (vel[ny-1]>2.0*kap1/dy)
             {
                   a[ny-1] = HalfTau*vel[ny-1]/dy;
                   b[ny-1] = 1.0 - HalfTau*perf*0.333333333333  - HalfTau*vel[ny-1]/dy - kap1/(vel[ny-1]*(0.5*dy + vel[ny-1]*HalfTau));
             }
             else
{*/
...
/*           if (vel[0]<(-2.0*kap1/dy))
               {
                  c[0] = -HalfTau*vel[0]/dy;
                  b[0] = 1.0 - HalfTau*perf*0.333333333333 + kap1/(vel[0]*(0.5*dy + vel[0]*HalfTau)) + vel[0]*HalfTau/dy;
               }
               else
				{*/
...
//               }
				...
				temperature.TriDiagMultiply( a,  b,  c, 2, ny,x,z); // the 2 refers to multiplication along the y index.
        }
    }
